...
void BaseCache::initDerivedParams() {
    ...
    offsetLength = (uint32_t)log2(blockSize/4);
    if (associativity != 0) {
...
bool BaseCache::write(uint32_t addr, uint32_t data) {
    ...
    if (cacheLines != NULL) {
        ...
        // If there is no match tag, find a LRU cache and write it.
        for (uint32_t i = 0; i < associativity; i++) {
            if (cacheLines[index_needed][i].LRUStackBits == 0) {
                MostRecentUsed(index_needed, i);
                cacheLines[index_needed][i].tag = tag_needed;
                for (uint32_t j = 0; j < blockSize/4; j++) {
                    if (j == offset_needed) {
                        cacheLines[index_needed][i].data[j] = data;
                    } else {
                        cacheLines[index_needed][i].data[j] = 0;
                    }
                }
                numWrites++;
                numWriteMisses++;
